...
while(1):
    dt = datetime.now()
    logger.info(f'Start processing {dt}')
    if dt.weekday() < 5 and dt.hour > 8 and dt.hour < 18:
        ...
        logger.info(f'Deadletters info: {str(result_dict)}')
        is_new_value = func.check_previous_version(result_dict)
        logger.info(f'Is new value: {is_new_value}')
        if len(result_dict) > 0 and is_new_value:
            func.sendEmail(func.generate_html_table(result_dict), cnfg['email_login'], cnfg['email_password'], cnfg['emails_to'], logger, '1')
    logger.info(f'Stop processing: {datetime.now()}. Waiting...')
    time.sleep(10800)